...
订单状态：`awaiting_payment → generating → ready/failed`，另有 `cancelled` 和 `expired`。文件状态：`queued → processing → ready/failed → expired`。`ready` 只代表 PDF 生成，绝不表示纸已打印。重复模拟支付只写入一条支付记录，订单生成可重试但不会真实重复出纸。
