...
before(async () => { await createFixtures('examples'); await guest.get('/api/device').expect(200); await other.get('/api/device').expect(200); await admin.post('/api/admin/login').send({ password: process.env.ADMIN_PASSWORD }).expect(200); await admin.put('/api/admin/printers/default').send({ name: '业务测试队列（模拟）' }).expect(200); await admin.put('/api/admin/onboarding').send({ action: 'identity', shopName: settings().shopName, deviceName: settings().deviceName, address: settings().address }).expect(200); await admin.put('/api/admin/onboarding').send({ action: 'pricing', simplexPrice: 20, duplexPrice: 20 }).expect(200); await admin.put('/api/admin/onboarding').send({ action: 'complete', accepting: true }).expect(200); });
