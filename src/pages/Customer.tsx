...
export function Customer() {
  ...
  return <><Header/><main className="customer-main"><div className="test-strip"><span className="dot"/> 体验模式 <span>自动模拟支付，不扣款；按顺序合并 PDF</span></div><section className="hero"><div className="eyebrow">PRINT YOUR NEXT CHAPTER</div><h1>上传一份文件，<br/>留下一份<span>清晰。</span></h1><p>文件、照片、学习资料，几步准备好。<br className="mobile-only"/> 从你的手机，走向纸面。</p><div className="station-pill"><span className={`status-dot ${shop?.accepting ? '' : 'offline'}`}/>{shop?.shopName || '纸间自助打印'}<span className="divider"/>{shop?.deviceName || '一号打印站'}<span className="station-mode">PDF 测试</span></div><div className="paper-art" aria-hidden="true"><div className="sheet-back"/><div className="sheet-main"><span className="sheet-mark">P.</span><div className="paper-lines"><i/><i/><i/></div><span className="paper-a4">A4<br/>YOUR IDEAS,<br/>ON PAPER.</span></div><div className="paper-stamp"><Check size={24}/></div><span className="art-caption">一份整齐，刚刚好。</span></div></section>
    ...
    {(error || shopError || fileError) && <Alert>{error || shopError || fileError}</Alert>}{shop && !shop.accepting && <Alert type="info">{shop.unavailableReason} 你仍可查看已有订单。</Alert>}
